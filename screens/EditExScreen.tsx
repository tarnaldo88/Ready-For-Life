...
type Props = StackScreenProps<NutStackList, 'EditEx'>;

import { NutStackList } from '@/navigation/NutStackNavigator';
import { useAuth } from '../context/AuthContext';

const EditExScreen: React.FC<Props> = ({ route, navigation }) => {
  const { ex } = route.params;
  const { user } = useAuth();
  const userId = user?.uid || 'guest';
  const [goalName, setGoalName] = useState(ex.title);
  const [goalInformation, setGoalInformation] = useState(ex.description || '');
  const [goalStartDate, setGoalStartDate] = useState<Date | undefined>(ex.startDate ? new Date(ex.startDate) : undefined);
  const [goalEndDate, setGoalEndDate] = useState<Date | undefined>(ex.endDate ? new Date(ex.endDate) : undefined);
  const [completed, setCompleted] = useState(!!ex.completed);
